    ...
    def get_crypto_name():
        if request.method == 'POST':
            ...
            if crypto_name == "new crypto":
                os.system("python3 coins-info-crawler.py")
                ...
                os.system("python3 coins-info-crawler")
                ...
                crypto_names = request.form['crypto_names']
                crypto_name = crypto_names
                os.system(f"python3 coin-info-crawler.py {crypto_name}")
                if os.path.isfile(f"{crypto_name}.json"):
                    with open(f"{crypto_name}.json", 'r') as f:
                        return Response(response=f.read(), status=200, mimetype="application/json")
                else:
                    with open("InvalidCoinName.json", 'r') as f:
                        return Response(response=f.read(), status=200, mimetype="application/json")
